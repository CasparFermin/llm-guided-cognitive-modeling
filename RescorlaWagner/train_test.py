...
for exp in experiments:
    ...
    # read the data
    train_np = np.load(f"../Data/PreProcData/{exp['name']}/proc_Train_{exp['experiment']}.npy")
    test_np = np.load(f"../Data/PreProcData/{exp['name']}/proc_Test_{exp['experiment']}.npy")
    ...
    # create and save image of loss over training iterations
    ...
    inv_df.to_csv(f'../Data/RescolaWagnerResults/TVTIndResults_{exp['name']}_{exp['experiment']}.csv', index=False)

df = pd.DataFrame(tot_data, columns=['name', 'experiment', 'train_nll', 'test_nll'])
print(df)
print(df['test_nll'].mean())
df.to_csv('../Data/RescolaWagnerResults/TVTResults_summary.csv')